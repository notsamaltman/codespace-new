...
`,
  ...
  java: `public class Main {
    public static void main(String[] args) {
        System.out.println("Hackvision");
    }
}
`,

  javascript: `// Collaborative coding session
console.log("Hackvision");
`,
};
...
const Index = () => {
  ...
  const defaultCode  = DEFAULT_CODE_BY_LANGUAGE["python"];

  useEffect(() => {
  if (!hasInitializedCodeRef.current) return;

  const template = DEFAULT_CODE_BY_LANGUAGE[language];
  if (!template) return;
  ...
  setParticipants(normalized);
    ...
    // Listen for full / incremental code history updates
newSocket.on("code-history", ({ history }) => {
  ...
  return (
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      <div className="border-b px-4 py-2 flex justify-between items-center">
        ...
        <Button
      onClick={handleSave}
      className="h-9 px-4 absolute left-32 bg-secondary text-secondary-foreground"
    >
      Save
    </Button>
